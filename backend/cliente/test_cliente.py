...
def novo_cliente():
    ...
    yield response
    client.delete(f"/clientes/clientes/{cliente_exemplo['id']}")

def test_adicionar_cliente():
    response = client.post("/clientes/clientes", json=cliente_exemplo)
    assert response.status_code == 200
    assert response.json()["id"] == cliente_exemplo["id"]
...
def test_listar_clientes():
    ...
    assert response.status_code == 200
    assert any(cliente["id"] == cliente_exemplo["id"] for cliente in response.json())

@pytest.mark.usefixtures("novo_cliente")
def test_buscar_cliente():
    response = client.get(f"/clientes/clientes/{cliente_exemplo['id']}")
    assert response.status_code == 200
    assert response.json()["id"] == cliente_exemplo["id"]
...
def test_atualizar_cliente():
    ...
    response = client.put(f"/clientes/clientes/{cliente_exemplo['id']}", json=novo_dado)
    assert response.status_code == 200
...
def test_remover_cliente():
    ...
    response = client.delete(f"/clientes/clientes/{cliente_exemplo['id']}")
    assert response.status_code == 200
